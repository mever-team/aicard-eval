...
print(emissions_out)
```
Energy is measured in `kWh` and emission in `kg CO₂eq`. If you are using this on a server, make sure you have only one instance of `CarbonTrack()` defined. You can initialize multiple runs with the same instance and track multiple runs at the same time.  
> [!WARNING]
